...
export default {
  ...
  login ({ commit, state, dispatch }, payload) {
    ...
    // Try to sigin
    firebase.auth().signInWithEmailAndPassword(payload.userDetails.studentNumber, payload.userDetails.password)
  ...
  registerUser ({dispatch}, payload) {
    ...
    firebase.auth().createUserWithEmailAndPassword(payload.userDetails.studentNumber, payload.userDetails.password)
      .then(() => {
        payload.notify({
  ...
  LoginStudent ({ commit }, payload) {
    ...
    return new Promise((resolve, reject) => {
      jwt.login(payload.userDetails.studentNumber, payload.userDetails.password)
        .then(response => {
  ...
  registerUserJWT ({ commit }, payload) {
    ...
    const { displayName, studentNumber, password, confirmPassword } = payload.userDetails

    return new Promise((resolve, reject) => {

      // Check confirm password
      if (password !== confirmPassword) {
        reject({message: '密码与上次输入不一致，请重新输入'})
      }
